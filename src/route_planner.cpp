...
bool CostCompareReverse(const RouteModel::Node *const a,
                        const RouteModel::Node *const b) {
  return (a->h_value + a->g_value) > (b->h_value + b->g_value);
}

RouteModel::Node *RoutePlanner::NextNode() {
  std::sort(open_list.begin(), open_list.end(), CostCompareReverse);
...
RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  ...
  RouteModel::Node *thru = current_node;

  path_found.emplace_back(*current_node);
  while (thru != nullptr && thru != start_node) {

    auto parent = thru->parent;
    if (parent != nullptr) {
      distance += thru->distance(*parent);
